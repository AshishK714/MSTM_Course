...
<p>The ten sections below work through those decisions in the order they were made. They build the same habit throughout: read the column name, then ask what had to happen before a value could be written underneath it.</p>
"""

s1 = sec('s1', '3.1', 'A column is a recorded thing whose form somebody chose', """
<p>Open the FPF file and compute the average attendance. It will not compute. The attendance_pct column is stored as text, because every value ends in a percent sign: "96%", "95%", "93%". The percent sign makes each value a string, and a string cannot be averaged. Strip the sign, convert to a number, and the mean is 93.13. The digital_lit_score column was stored the same way.</p>
<p>Nothing is wrong with the file. Somebody chose to record the percent sign along with the number, which is a reasonable thing to do if the column is meant to be read by a person. It stops being reasonable as soon as anyone wants to compute with it, and whoever made the choice was not thinking about that.</p>
<p>The general point is that a column is not a number. It is a record of something, in a form that somebody selected, and the form decides which operations are legal. Text can be counted and matched. Numbers can be added and averaged. Ordered categories can be ranked, and the gaps between them are not known to be equal. The same underlying fact can be stored in any of these forms, and the choice is usually made for convenience at the point of recording.</p>
<p>This is the cheapest of all the problems in this part, because a tool will refuse outright and you will notice within a minute. The remaining nine sections are about the ones where the tool computes something and produces no warning.</p>
""" + ask('What form is this value stored in, and did somebody choose that form for a purpose other than mine?')
+ src('the in-class attempt to average attendance; Reading Note 3 on variable types.'))

s2 = sec('s2', '3.2', 'A structural blank is not missing data', """
...
s4 = sec('s4', '3.4', 'What can be added up', """
...
<p>The last row is the case that misleads. Anyone who tried this method only on partnership strength would conclude it was sound, and would carry it to the next question, where the groups are not equal.</p>
...
s5 = sec('s5', '3.5', 'The chain of decisions behind one column', """
...
s6 = sec('s6', '3.6', 'What a measure records and what it leaves out', """
...
s7 = sec('s7', '3.7', 'Proxies, and why being recordable is not being a good measure', """
<p>Four columns in the FPF file were put in front of the class with the question of how much to trust each one. Three of them stand in for something that was never measured. Each was then checked against whether the participant was placed.</p>
...
s9 = sec('s9', '3.9', 'Comparability', """
<p>Two learners are assessed before and after a programme.</p>
""" + table(['Learner', 'Before', 'After', 'National percentile'], [
    ['A', '90', '92', '95'],
    ['B', '55', '78', '70'],
]) + """
<p>Which is the stronger candidate? The question cannot be answered as posed. An employer selecting on current capability should prefer A. Somebody evaluating the teaching should notice that B gained twenty-three points against two. A scholarship committee weighing distance travelled may reach a third conclusion. The four numbers do not change, and the correct summary changes with the decision.</p>
...
s10 = sec('s10', '3.10', 'Write it down', """
...
<p>The place it belongs is a data dictionary: one entry per column, saying what the column is meant to represent, how it was collected, what its values mean, and, for a proxy, the three statements owed under 3.7. The FPF case came with one.</p>
<p>Derived columns need this most, because they have no existence outside the decision that made them. A readiness score built by averaging two percentages is exactly the averaging in 3.8, and six months later the only difference between a defensible column and an indefensible one is whether somebody wrote down how it was built. Write the entry when you build the column, because that is the only moment at which you still know.</p>
...
""" + kc(P, 1,
   'A tool refuses to compute the mean of attendance_pct. The values look like 96%, 95%, 93%. What is the problem?',
   ['The column has blanks that the tool cannot handle',
    'The percent sign makes every value text',
    'Percentages cannot be averaged under any circumstances',
    'The column was recorded on a scale that has no true zero'],
   1, 'The percent sign makes every value a string. Strip it, convert, and the mean is 93.13.') \
+ kc(P, 2,
...
+ resp(
   'The FPF director, a funder, and a prospective participant each ask how well the programme places people. Give each of them one of the three figures from 3.3, say why that one, and say what each would wrongly conclude from one of the other two.',
...
PAGE = {
    ...
    'toc': [('s1', '3.1 The form somebody chose'), ('s2', '3.2 Structural blanks'),
            ('s3', '3.3 Denominators'), ('s4', '3.4 What can be added up'),
            ('s5', '3.5 The chain of decisions behind one column'),
            ('s6', '3.6 What a measure leaves out'), ('s7', '3.7 Proxies'),
            ('s8', '3.8 Scale'), ('s9', '3.9 Comparability'), ('s10', '3.10 Write it down'),
            ('practice', 'Practice')],
    'body': intro + s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8 + s9 + s10 + practice,
