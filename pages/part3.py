...
<p>The test is to find the column that controls the blank. If the blanks line up exactly with the values of another column, they are structural, and they should be left alone and explained. If they do not line up with anything, the value was genuinely lost in collection, and that needs the opposite treatment: find out why it is missing, report the smaller group the figures now describe, and do not fill it in without saying so.</p>
...
<p>The standard deviation is a measure of spread, roughly the typical distance of a value from the mean of its column, and Part 5 builds it. What matters here is the comparison: 11.88 against 5.21, so digital literacy varies more than twice as widely as attendance.</p>
<p>In an average of two columns, the share of the variation each one contributes goes by the square of its spread, not by the spread itself. Squaring gives 141 against 27, and 141 of 168 is about 84 percent. So a learner's digital literacy moves the readiness score and their attendance barely does. Nobody intended to weight one column that heavily, and a reader of the score cannot tell from it that this happened.</p>
<p>There is a second problem, and it is separate from the spread. Both columns end in a percent sign, and neither column name records what the percent is of. Attendance is a percentage of sessions available. Digital literacy is a percentage of marks on an assessment. Two columns that use the percent sign are therefore not measuring the same kind of thing, whatever their spreads.</p>
<p>The same warning applies to ordered categories. Code employer_partnership_strength from 1 to 4 and the mean is 2.57. There is no partnership of strength 2.57, and the figure depends on codes nobody chose: recode the four levels as 1, 2, 4 and 8 and the ranking of any two groups can change, while nothing about the partnerships has. That is what averaging assumes and cannot get, that the step from Very Low to Low is the same size as the step from High to Very High. Separately, the placement rates across the four levels run 40.88, 36.99, 36.21 and 38.78, so the order does not even track the outcome.</p>
""" + ask('Do these columns vary by similar amounts, and is each one a proportion of the same kind of thing?')
...
+ src('Reading Note 2, section 9, for the two learners; the in-class activity on rural and urban salaries. Part 4 covers who ended up in the rural group; Part 5 covers what happens to this comparison when education is held still, in a section that is not on the midterm.'))
