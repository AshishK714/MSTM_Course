...
<p>The chain runs one way. Rows recorded by customer and day can be folded into rows by customer whenever you want, folded meaning summarised so that several rows become one. The reverse is impossible. Once the file holds one row per customer, the days are gone, and no technique can recover them.</p>
...
<p>Now put the first question to that second table. Do mentored learners place better? The three-row table cannot answer it. Each of its rows holds mentored and unmentored learners together. Inside the 2,091 Hybrid learners, the 276 who went unmentored were employed at 39.49 percent and the 1,815 who were mentored at 40.11, and the 40.0 percent on the track row is those two weighted by their sizes. Once the row is written, the two figures behind it are gone. You put in 5,000 learner rows and got back three, which is the one-way property from 2.1 applied to a table rather than to a file. Nothing was lost from the file, and the repair is to go back and build a different table.</p>
<p>This is the practical form of the rule. A table is adequate for a comparison when the thing being compared still varies inside it. The mentorship question needs learner rows, because mentorship varies between learners. The track question is answered by the three-row table, because track still varies across its rows. Mentorship does not, which is why the same table cannot answer the first question. Each level supports a comparison the others cannot, and no level is the correct one in general.</p>
...
<p>Once the level of detail follows from the comparison, the question becomes at which rung a decision about mentorship should be made. There are three rungs, and the file supports two of them.</p>
<p>At programme level FPF decides one thing: run mentorship for everyone, or for nobody. Two numbers are enough to decide it. Among the 4,363 mentored participants, 38.90 percent ended up employed, against 33.75 percent of the 637 unmentored, a gap of 5.15 percentage points. Every participant is affected by that decision, and no characteristic of any particular participant was used in making it.</p>
<p>At group level the decision is still made per participant, by virtue of the group they belong to, and the point of splitting is that it tells you where to spend a limited budget: you fund the groups where the gap per pound is largest. Splitting the participants by education gives three separate gaps.</p>
...
<p>At individual level the file cannot support a decision. To decide correctly for one named participant you would need to know what happened to that participant both with mentorship and without it. The second is the counterfactual, the thing that did not happen, and only one of the two was ever observed, so there is nothing to compare.</p>
<p>There is one condition on deciding at group level, and it rules out several groups here. A group is usable only if it contains both mentored and unmentored members. All 637 unmentored participants are urban, all have one to three years of prior experience, and all have mid-level AI exposure. That has a second consequence, larger than the first: the 637 are a narrow slice of the cohort, so the headline gap of 5.15 points is comparing them with 4,363 people who are not like them in several recorded ways. Rural learners, those with four years or more, and those at low or high AI exposure therefore have no unmentored members at all. For those groups the file contains no comparison.</p>
""" + lost('Slice a file enough ways and some group will show a gap, because small groups bounce around on their own. A gap of nine points among 251 people and the same gap among twenty are not the same finding. The exam does not ask you to test whether a gap is real; it asks you to notice how many people are behind it before you rely on it.')
...
<p>Conditioning means restricting to one value of a column and computing inside that restriction, and the column you restrict on becomes the denominator. All three directions condition on something different, which is why they differ.</p>
<p>The grand percentages divide every cell by the same 5,000, so they describe the composition of the cohort and compare nothing. The column percentages are close together, 88.76 against 86.33, because 4,363 of 5,000 participants were mentored, which is 87.26 percent, so any group drawn from this file starts near 87 percent mentored. The 2.4 points between them is the mentorship signal, squeezed onto a scale where it is nearly invisible and easy to misread as mentorship being everywhere among the employed.</p>
...
<p>The words row and column belong to the layout you happened to build, and transposing the table renames them without changing which denominator your decision needs. What the data does not record is which denominator was used, so a percentage arrives with no label saying what it is a percentage of.</p>
...
<p>The answer is in the columns. One column, campaign, records how many times this customer was contacted during this campaign. Read that as a test of what a row is. If one row were one call, a customer contacted five times would occupy five rows, the contact count would run from one to five across them, and the customer's identifier would repeat. Neither happens. The count arrives once, as a single value on a single row, which is only sensible if that row already stands for the whole sequence of calls to that customer. The assignment's own column list settles the rest: the row holds the last contact of the campaign, with the earlier attempts summarised into a count.</p>
<p>Getting this right changes which questions the file can answer. You can ask whether mobile beats landline for a customer, with one caution: the channel recorded is the channel of that last contact, so the comparison treats it as the customer's channel and is only as good as that assumption. You cannot ask whether the third call converts better than the first, because the first two calls are not rows. They were folded into a count before you received the file, and the rule in 2.1 applies: the folding cannot be undone.</p>
...
<p>Row granularity is what one row represents, and it follows from the comparison (2.2). It is the same thing the reading notes call the grain.</p>
<p>Column granularity is how finely a value is recorded. Most things vary continuously, and we record them at separated points because that is cheaper. How finely you record decides which kind of variable you have, and how fine you need it depends on how you plan to use it. Continuous values fall anywhere on a range, discrete ones land on separated points such as whole years, ordinal ones are groups with an order but no fixed distance between them, and categorical ones are groups with no order at all.</p>
<p>Age was the class example. In years and months it is continuous. Rounded to whole years it is discrete. Put into bands it is categorical, and because those bands have an order it is more exactly called ordinal. Each step loses variation: held in whole years, half a year, three quarters and a full year are all recorded as one year. The skill, effort and knowledge a student brings to an exam vary continuously, and the recorded result may be only a pass or a fail, which is a discrete record with two categories.</p>
<p>Coarser is not always worse. The Titanic example in class was a case where banding is right. Age and survival are related in a complicated way, one way up to about twenty and the other above fifty. Suppose a person had to be at least fourteen to swim. Ability to swim then rises sharply at fourteen, and survival with it. A band with its edge at fourteen matches something real, as do ages fixed by law, such as driving.</p>
...
]) + '<p>Two other facts are in the file and not in this table. Across all 5,000 participants, 87.26 percent were mentored. Across those same 5,000, every one of the 637 who went without mentorship is urban, has one to three years of prior experience, and has mid-level AI exposure.</p>') + """
