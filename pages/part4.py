...
<p>Among the 1,912 placed participants, the 215 from rural areas earn an average salary of 68,172. The 1,697 from urban areas earn an average of 62,503. The rural figure is higher by 5,669. Rural participants are also placed more often, 41.51 percent of 518 against 37.86 percent of 4,482, so they look better on both measures.</p>
...
<p>The reason is in how the current rural learners came to be in the file. Every one of them travelled to a city centre to attend, and paid for that in time, money and effort. Only some rural people were willing to pay that cost, so the cost decided who enrolled. The rural people who did enrol are not typical rural people. They are the ones who could bear the cost: the ones with the most motivation, the most support at home, or the fewest other demands on their time. That is an assumption, and it is the assumption that makes the prediction point downwards. If travel had instead selected people who place worse, removing it would raise the rate. The file contains the people who passed the filter. It contains nothing about the people who did not.</p>
...
<p>This is called survivorship bias. The name comes from situations where only the survivors of some process are available to study, and the cases that did not survive are not there to be counted. The mechanism is the same as in 4.2: some cases were removed before the file was built, and which ones were removed depended on the thing being measured. That last condition is what does the damage. A portal that closed every third contact at random would also shrink the file, and it would not bias the comparison at all, because the cases it removed would look like the ones it left.</p>
<p>So survivorship is a kind of selection: in both, who or what got into the file depended on the thing being measured. The two differ in direction. In 4.2 the filter let unusual people in. Here the filter took ordinary cases out. In both cases the file describes a narrower group than its name suggests.</p>
